...
def prepare_data_Rain12600(data_path, patch_size, stride):
    ...
    data_path = 'D:\\NH_Networks'
    input_path = data_path + '\\' + 'rainy_image'
    #input_path = os.path.join(data_path, '/', 'rainy_image')
    target_path = data_path + '\\' + 'ground_truth'
    #target_path = os.path.join(data_path, '/', 'ground_truth')

    save_target_path = os.path.join(data_path, 'train_target.h5')
    #save_target_path = data_path + '\\' + 'train_target.h5'
    print("어기는 어디인가 ? : ", data_path)
    print("어우야 : ", save_target_path)
    save_input_path = os.path.join(data_path, 'train_input.h5')
    #save_input_path = data_path + '\\' + 'train_input.h5'
    ...
    train_num = 0
    for i in range(28):
        #target_file = "frame%03d.jpg" % ((i))
        target_file = "frame%d.jpg" % ((i*100))
        print("체크1 : ", (target_file))
        target = cv2.imread(target_path + '\\' + target_file)
        print(np.shape(target))
        print("체크2 : ", (target_path + '\\' + target_file))
        b, g, r = cv2.split(target)
        target = cv2.merge([r, g, b])

        for j in range(1):
            #input_file = "frame%d.jpg" % ((i * 26) + 6954)
            input_file = "frame%d.jpg" % ((i*100))
            input_img = cv2.imread(input_path + '/' + input_file)
